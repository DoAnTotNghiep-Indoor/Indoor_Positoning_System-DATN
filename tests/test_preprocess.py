...
from ml.preprocess import coords, denoise, filter as ap_filter, missing, scale, split
...
def test_loc_ap_theo_nguong(bang_van_tay):
    _, giu, ty_le = ap_filter.filter_access_points(bang_van_tay, AP_COLS, min_appear_rate=0.2)
...
def test_nguong_khong_loai_ai_khi_bang_khong(bang_van_tay):
    _, giu, _ = ap_filter.filter_access_points(bang_van_tay, AP_COLS, min_appear_rate=0.0)
    assert len(giu) == 3


def test_nguong_qua_cao_thi_bao_loi(bang_van_tay):
    with pytest.raises(ValueError, match="loại sạch"):
        ap_filter.filter_access_points(bang_van_tay, AP_COLS, min_appear_rate=1.01)
...
def test_thu_tu_cot_ap_luon_duoc_sap_xep(bang_van_tay):
    ...
    _, giu, _ = ap_filter.filter_access_points(dao_nguoc, AP_COLS[::-1], min_appear_rate=0.0)
...
def test_loai_mau_qua_it_ap(bang_van_tay):
    ket_qua, tk = ap_filter.filter_sparse_scans(bang_van_tay, AP_COLS, min_ap_per_scan=3)
...
def test_gia_tri_dien_thieu_la_min_tru_mot(bang_van_tay):
    gia_tri = missing.compute_missing_value(bang_van_tay, AP_COLS)
    assert gia_tri == pytest.approx(np.nanmin(bang_van_tay[AP_COLS].to_numpy()) - 1)


def test_dien_het_o_trong(bang_van_tay):
    ket_qua, gia_tri, so_o = missing.fill_missing(bang_van_tay, AP_COLS)
...
def test_hampel_thay_ngoai_lai_bang_trung_vi():
    ...
    ket_qua, so_thay = denoise.hampel_filter(df, ["AP"])
    assert so_thay == 1
...
def test_hampel_giu_nguyen_so_dong():
    ...
    ket_qua, _ = denoise.hampel_filter(df, ["AP"])
...
def test_hampel_bo_qua_nhom_khong_bien_thien():
    ...
    _, so_thay = denoise.hampel_filter(df, ["AP"])
    assert so_thay == 0
...
def test_bo_mau_thieu_toa_do(bang_van_tay):
    ...
    ket_qua, tk = coords.attach_coordinates(bang_van_tay, rp, policy="drop")
    assert tk["rp_chua_do"] == ["RP04"]
    assert tk["scan_bi_bo"] == 5
    assert ket_qua["x"].notna().all()


def test_chinh_sach_error_thi_dung_lai(bang_van_tay):
    rp = pd.DataFrame({"rp_id": ["RP01"], "x": [0.0], "y": [0.0]})
    with pytest.raises(ValueError, match="chưa có toạ độ"):
        coords.attach_coordinates(bang_van_tay, rp, policy="error")


# --- Bước 9 + 10: chia tập và chuẩn hoá ---

def test_chia_tap_du_ba_phan(bang_van_tay):
    ket_qua, tk = split.split_dataset(bang_van_tay, strategy="random")
    assert set(ket_qua["split"]) == {"train", "validation", "test"}
    assert len(ket_qua) == len(bang_van_tay)
    assert tk["chien_luoc"] == "random"


def test_device_holdout_bao_loi_khi_mot_thiet_bi(bang_van_tay):
    with pytest.raises(ValueError, match="máy thứ hai"):
        split.split_dataset(bang_van_tay, strategy="device_holdout")


def test_chien_luoc_la_bao_loi(bang_van_tay):
    with pytest.raises(ValueError, match="không hợp lệ"):
        split.split_dataset(bang_van_tay, strategy="khong_ton_tai")
...
def test_scaler_chi_hoc_tu_train():
    ...
    ket_qua, scaler, _ = scale.scale_dataset(df, ["AP"])
...
def test_khong_co_train_thi_bao_loi():
    df = pd.DataFrame({"rp_id": ["RP01"], "split": ["test"], "AP": [1.0]})
    with pytest.raises(ValueError, match="train"):
        scale.scale_dataset(df, ["AP"])
...
def test_du_lieu_that_du_cot_bat_buoc():
    from ml.preprocess import load
